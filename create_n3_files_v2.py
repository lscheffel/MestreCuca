#!/usr/bin/env python3
"""Script para gerar arquivos N3 a partir da Taxonomia Ontológica - Versão 2."""
...
# Encontra todas as secoes N3 na tabela
# Formato: | **N3** | **1.1.1** | **RESOLUÇÃO** | ...
pattern = r'\| \*\*N3\*\* \| \*\*(\d+\.\d+\.\d+)\*\* \| \*\*(\w+)\*\* \| ([^|]+) \|'

matches = re.finditer(pattern, content)

n3_list = []
for match in matches:
    n3_id = match.group(1)  # 1.1.1
    nome = match.group(2)   # RESOLUCAO
    foco = match.group(3).strip()
    
    # Determina o dominio pai baseado no primeiro digito
    ...
    n2_id = f"{n2_prefix}.{n3_id.split('.')[1]}"
    
    # Busca a descricao detalhada da secao correspondente
    # Procura por "#### 3.3.X [N2] NOME:" ou similar
    secao_pattern = rf'#### (\d+\.\d+\.\d+) \[{n2_id}\] {nome}:(.*?)(?=\n#### |\n---|\n## |$)'
    secao_match = re.search(secao_pattern, content, re.DOTALL)
    
    foco_desc = foco
    crenca = "Não especificada"
    tags_herdadas = ""
    
    if secao_match:
        secao_texto = secao_match.group(2)
        # Extrai foco
        foco_match = re.search(r'\*\*foco:\*\* "([^"]+)"', secao_texto)
        if foco_match:
            foco_desc = foco_match.group(1)
        # Extrai crenca
        crenca_match = re.search(r'\*\*crença:\*\* "([^"]+)"', secao_texto)
        if crenca_match:
            crenca = crenca_match.group(1)
        # Extrai tags
        tags_match = re.search(r'\*\*Tags Herdadas:\*\* \[([^\]]+)\]', secao_texto)
        if tags_match:
            tags_herdadas = tags_match.group(1)
...
print(f"Encontrados {len(n3_list)} subdomínios N3")
...
print(f"\nTotal de arquivos criados: {len(n3_list)}")
