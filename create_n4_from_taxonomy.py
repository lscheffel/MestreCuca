#!/usr/bin/env python3
"""Script para gerar células N4 a partir da Taxonomia Ontológica - versão corrigida."""
...
# Encontra todas as células N4 - procurando por "Célula XXX-X:"
# O formato é: **Célula XXX-X: NOME** (em negrito)
pattern = r'\*\*Célula (\w+-\w+):\*\* ([^\n]+)\s*\n-\*\*Gatilho:\*\* ([^\n]+)\s*\n-\*\*Ação:\*\* ([^\n]+)\s*\n-\*\*Restrição:\*\* ([^\n]+)\s*\n-\*\*Verificação:\*\* ([^\n]+)'

matches = list(re.finditer(pattern, content))
print(f"Encontradas {len(matches)} células N4 via regex 1")

# Se não encontrar, tenta outro padrão (sem negrito no nome)
if len(matches) == 0:
    pattern = r'Célula (\w+-\w+): ([^\n]+)\s*\n- Gatilho: ([^\n]+)\s*\n- Ação: ([^\n]+)\s*\n- Restrição: ([^\n]+)\s*\n- Verificação: ([^\n]+)'
    matches = list(re.finditer(pattern, content))
    print(f"Encontradas {len(matches)} células N4 via regex 2")

n4_list = []
for match in matches:
    cell_id = match.group(1)  # R1.1.1-A
    nome = match.group(2).strip()     # DECOMPOSIÇÃO_BINÁRIA
    ...
    # Busca o subdomínio N3 correspondente na tabela
    # Procura por padrões como: | **N3** | **1.1.1** | **RESOLUÇÃO** |
    n3_pattern = rf'\| \*\*N3\*\* \| \*\*{re.escape(n3_num)}\*\* \| \*\*(\w+)\*\* \|'
    n3_match = re.search(n3_pattern, content)
    
    n3_nome = "DESCONHECIDO"
    n2_id = "0.0"
    if n3_match:
        n3_nome = n3_match.group(1)
        # Extrai N2 ID do n3_num (ex: 1.1.1 -> 1.1)
        n2_parts = n3_num.split('.')
        n2_id = f"{n2_parts[0]}.{n2_parts[1]}"
    else:
        # Tenta encontrar na seção de descrição
        n3_pattern2 = rf'#### (\d+\.\d+\.\d+) \[(\d+\.\d+)\] (\w+):'
        n3_matches = re.finditer(n3_pattern2, content)
        for n3m in n3_matches:
            if n3m.group(1) == n3_num:
                n2_id = n3m.group(2)
                n3_nome = n3m.group(3)
                break
...
print(f"Total de células N4 processadas: {len(n4_list)}")

# Mostra alguns exemplos
for cell in n4_list[:3]:
    print(f"  - {cell['cell_id']}: {cell['nome']} (N3: {cell['n3_ref']} {cell['n3_nome']})")
...
print(f"\nSubdomínios N3 com células: {len(n3_groups)}")
